...
function StepItem({ step, index }) {
  ...
  return (
    <div ref={ref} className="min-h-[50vh] flex items-center relative py-16">
      ...
      >
        ...
        <div className="relative z-10">
          <span className={`inline-block border border-cream/10 bg-ink-950 font-mono text-xs px-3 py-1 rounded-full mb-6 transition-colors duration-500 ${inView ? 'text-signal border-signal/30' : 'text-mist-900'}`}>
            {step.tag}
          </span>
    ...
    <section ref={containerRef} className="relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-24 pb-8">
        <h3 className="font-display text-3xl md:text-5xl font-bold tracking-tight">
          Como é começar com a <span className="text-signal">Nexus</span>
        </h3>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12 relative w-full">
        ...
        <div className="pb-24">
          {onboardingSteps.map((step, index) => (
            <StepItem key={step.tag} step={step} index={index} />
          ))}
        </div>
      </div>
    </section>
